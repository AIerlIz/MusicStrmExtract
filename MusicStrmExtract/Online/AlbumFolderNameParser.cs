...
{
    private static readonly Regex s_yearSuffixRegex = new(
        @"[\s_\-\.]*[\(\[（【]?\s*(18|19|20)\d{2}\s*[\)\]）】]?\s*$",
        RegexOptions.Compiled);

    private static readonly Regex s_trailingSeparatorRegex = new(
        @"[\s\-\._]+$",
        RegexOptions.Compiled);

    private static readonly Regex s_folderYearRegex = new(
    ...
    public static string? Clean(string? raw, string? artistName = null)
    {
        ...
        var s = trimmed;
        s = s_yearSuffixRegex.Replace(s, string.Empty);
        s = s_trailingSeparatorRegex.Replace(s, string.Empty);
    ...
    public static int? ParseYear(string? albumFolderName, string? artistName)
    {
        ...
        var match = s_folderYearRegex.Match(trimmed);
