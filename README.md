...
- `修复旧库专辑关系`：删除无路径、无 MusicBrainzAlbum 且未被 Audio 引用的陈旧 MusicAlbum，并刷新缺失或陈旧专辑关联的 `.strm`。
- `清除插件缓存`：清空进程内的专辑定位缓存和专辑目录扫描缓存，不影响 Emby 媒体库数据。
- `测试 MusicBrainz 连接`：使用当前配置的服务地址执行一次最小搜索，检查网络和镜像可用性。
- `刷新定位诊断`：显示本次 Emby 进程内最近的成功、未命中和来源异常记录。诊断数据不持久化。
