...
- 21 条 strm(周杰伦《七里香》《叶惠美》)全量:探测 21/21 成功,写回 21,封面落盘 21,失败 0。
- 库扫描后自动生成 2 个 `MusicAlbum` + 1 个 `MusicArtist`,专辑封面(600×600)已挂载。
- **幂等**:重复运行写回变更 = 0,无副作用。
...
2. 重启 Emby Server。
3. 确认:管理页 → 计划任务 → “Music Strm 元数据提取”(默认每周自动一次,也可手动运行)。
...
MusicStrmExtract/
  ...
  Tasks/      MusicStrmScanTask(计划任务,仅 music 库)
